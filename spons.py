...
async def init_db():
    try:
        ...
        # username/chat_id indexleri sparse+unique olmalı (ikisinden biri boş olabilir).
        # Eski index farklı ayarlarla varsa çakışmasın diye önce siliniyor.
        try:
            existing_idx = await col_post_channels.index_information()
            if "username_1" in existing_idx and not existing_idx["username_1"].get("sparse"):
                await col_post_channels.drop_index("username_1")
        except Exception:
            pass
        await col_post_channels.create_index("username", unique=True, sparse=True)
        await col_post_channels.create_index("chat_id", unique=True, sparse=True)
...
async def add_post_channel(name, identifier):
    """
    identifier: "@username" / "username" ya da sayısal chat ID (mesela -1001234567890) olabilir.
    Dönüş: (True, None) başarılıysa, (False, hata_mesajı) hatalıysa.
    """
    identifier = identifier.strip()

    # Sayısal ID mi? (örn: -1001234567890 ya da 123456789)
    if re.fullmatch(r"-?\d+", identifier):
        chat_id = int(identifier)
        await col_post_channels.update_one(
            {"chat_id": chat_id},
            {"$set": {"name": name, "chat_id": chat_id}, "$unset": {"username": ""}},
            upsert=True
        )
        return True, None

    # Aksi halde @username olarak kabul et
    uname = identifier.lstrip("@").strip()
    if not uname:
        return False, "Geçersiz ID ya da username."

    await col_post_channels.update_one(
        {"username": uname},
        {"$set": {"name": name, "username": uname}, "$unset": {"chat_id": ""}},
        upsert=True
    )
    return True, None
...
async def pch_add_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "➕ <b>Chat Ekleme</b>\n\n"
        "Введите в формате:\n"
        "<code>Название ID_или_@username</code>\n\n"
        "Примеры:\n"
        "<code>MyChannel @mychannel</code>\n"
        "<code>MyChannel -1001234567890</code>\n\n"
        "ℹ️ Приватные чаты/каналы только по ID добавляются (бот должен быть админом).\n\n"
        "Отмена: /cancel"
    )
...
async def process_post_channel_name(message: Message, state: FSMContext):
    ...
    if len(parts) < 2:
        await message.answer(
            "❌ Формат: <code>Название ID_или_@username</code>\n\n"
            "Примеры:\n"
            "<code>MyChannel @mychannel</code>\n"
            "<code>MyChannel -1001234567890</code>"
        )
    ...
    ok, err = await add_post_channel(name, identifier)
    if not ok:
        await message.answer(f"❌ {err}")
        return
    
    label = identifier if identifier.startswith("@") or re.fullmatch(r"-?\d+", identifier) else f"@{identifier}"
    await message.answer(
        f"✅ Чат <b>{name}</b> ({label}) добавлен!"
    )
...
async def pch_send_all(call: CallbackQuery, state: FSMContext):
    ...
    names = ", ".join(f"@{c['username']}" if c.get("username") else f"ID:{c.get('chat_id')}" for c in channels)
    await call.message.edit_text(
...
async def pch_send_one(call: CallbackQuery, state: FSMContext):
    ...
    ch_label = f"@{ch['username']}" if ch.get("username") else f"ID:{ch.get('chat_id')}"
    await call.message.edit_text(
        f"📺 <b>{ch_label}</b> каналына пост отправьте:\n\n"
