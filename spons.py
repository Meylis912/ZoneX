...
# ╚══════════════════════════════════════════════════════════╝
# IS_RENDER=true bolsa MongoDB ulanylýar (Render-de disk ýatda saklanmaýar,
# şonuň üçin SQLite faýly her täzeden başlananda öçýär). Pydroid/lokal ýerde
# IS_RENDER goýulmasa, SQLite ulanylýar.
...
try:
    ...
    mongo_db = mongo_client["zonexpremsponsor_db"]
...
USE_MONGO = IS_RENDER and mongo_db is not None

def get_next_id(counter_name):
    """Mongo-da auto-increment ID almak üçin (sponsors/addlists id-leri üçin)."""
...
app = Flask(__name__)
RENDER_URL = os.getenv("RENDER_URL", "https://zonexpremsponsor.onrender.com")
...
# Botuň sazlamalary
BOT_TOKEN = '7941701421:AAGfTRqxoB16P8vDfVkEBvCCluKvxRm_5Xw'
...
def get_user_language(user_id):
    if USE_MONGO:
        try:
            doc = mongo_db["settings"].find_one({"_id": f"lang_{user_id}"})
            return doc["value"] if doc else 'ru'
        except Exception as e:
...
def get_tgrass_enabled():
    if USE_MONGO:
        try:
            doc = mongo_db["settings"].find_one({"_id": "tgrass_enabled"})
...
def init_db():
    if USE_MONGO:
        try:
            mongo_db["settings"].update_one(
                {"_id": "start_text"}, {"$setOnInsert": {"value": ""}}, upsert=True
            )
            mongo_db["settings"].update_one(
                {"_id": "vpn_code"}, {"$setOnInsert": {"value": ""}}, upsert=True
            )
            mongo_db["settings"].update_one(
                {"_id": "tgrass_enabled"}, {"$setOnInsert": {"value": "1"}}, upsert=True
            )
            logging.info("✅ MongoDB collections ready")
        except Exception as e:
...
def get_setting(key):
    if USE_MONGO:
        try:
            doc = mongo_db["settings"].find_one({"_id": key})
...
def get_sponsors():
    if USE_MONGO:
        try:
            docs = mongo_db["sponsors"].find().sort("position", 1)
            return [(d["_id"], d["channel_id"], d["link"], d["position"]) for d in docs]
        except Exception as e:
            logging.error(f"Mongo error: {e}")
            return []
    with closing(sqlite3.connect('Fearsponsorbot.db')) as conn:
...
def get_addlists():
    if USE_MONGO:
        try:
            docs = mongo_db["addlists"].find().sort("position", 1)
            return [(d["_id"], d["name"], d["link"], d["position"]) for d in docs]
        except Exception as e:
            logging.error(f"Mongo error: {e}")
            return []
    with closing(sqlite3.connect('Fearsponsorbot.db')) as conn:
...
def get_all_users():
    if USE_MONGO:
        try:
            return [d["_id"] for d in mongo_db["users"].find()]
        except Exception as e:
            logging.error(f"Mongo error: {e}")
            return []
    with closing(sqlite3.connect('Fearsponsorbot.db')) as conn:
...
def is_valid_channel_url(link):
    """Buton URL'i olarak kullanılabilir mi kontrol eder (http/https ve geçerli host)."""
    if not link or not isinstance(link, str):
        return False
    return bool(re.match(r'^https?://[a-zA-Z0-9\-\.]+(:\d+)?(/.*)?$', link.strip()))
...
async def get_all_channels(user_id, username=None, is_premium=False):
    ...
    for sponsor in sponsors:
        if sponsor[2] not in used_urls and sponsor[3] is not None:
            if not is_valid_channel_url(sponsor[2]):
                logging.warning(f"Geçersiz sponsor linki atlandı: id={sponsor[0]} link={sponsor[2]!r}")
                continue
    ...
    for addlist in addlists:
        if addlist[2] not in used_urls and addlist[3] is not None:
            if not is_valid_channel_url(addlist[2]):
                logging.warning(f"Geçersiz addlist linki atlandı: id={addlist[0]} link={addlist[2]!r}")
                continue
    ...
    if tgrass_enabled:
        try:
            # requests senkron olduğu için event loop'u bloklamasın diye ayrı thread'de çalıştırıyoruz
            tgrass_offers = await asyncio.wait_for(
                asyncio.to_thread(check_tgrass_subscriptions, user_id, username, is_premium),
                timeout=6
            )
        except asyncio.TimeoutError:
            logging.warning("TGrass API timeout - devam ediliyor")
            tgrass_offers = []
        except Exception as e:
            logging.error(f"TGrass thread error: {e}")
            tgrass_offers = []
        ...
        if tgrass_offers:
            ...
            for i, offer in enumerate(tgrass_offers):
                ...
                if not is_valid_channel_url(offer_link):
                    logging.warning(f"Geçersiz tgrass linki atlandı: {offer_link!r}")
                    continue
...
async def cmd_start(message: Message):
    ...
    icon_custom_emoji_id=EMOJI_IDS["chanel"]
        ...
        logging.exception(f"/start hata: {e}")
        print(f"❌ /start HATASI: {e}")
        try:
            await message.answer("⚠️ Bir hata oluştu, admin bilgilendirildi.")
        except Exception:
            pass
...
async def process_sponsor_link(message: Message, state: FSMContext):
    ...
    if USE_MONGO:
        try:
            last = mongo_db["sponsors"].find_one(sort=[("position", -1)])
            new_position = (last["position"] + 1) if last else 1
            new_id = get_next_id("sponsors")
            mongo_db["sponsors"].insert_one({
                "_id": new_id,
...
async def delete_sponsor(call: CallbackQuery):
    ...
    if USE_MONGO:
        try:
            mongo_db["sponsors"].delete_one({"_id": sponsor_id})
            await call.message.edit_text(
...
async def process_addlist_link(message: Message, state: FSMContext):
    ...
    if USE_MONGO:
        try:
            last = mongo_db["addlists"].find_one(sort=[("position", -1)])
            new_position = (last["position"] + 1) if last else 1
            new_id = get_next_id("addlists")
            mongo_db["addlists"].insert_one({
                "_id": new_id,
                "name": name,
                "link": link,
                "position": new_position
...
async def delete_addlist(call: CallbackQuery):
    ...
    if USE_MONGO:
        try:
            mongo_db["addlists"].delete_one({"_id": addlist_id})
            await call.message.edit_text(
...
async def main():
    ...
    print("🤖 Бот работает...")
    print(f"💾 Storage: {'MongoDB (Render)' if USE_MONGO else 'SQLite (Local)'}")
